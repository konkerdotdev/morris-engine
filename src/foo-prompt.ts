...
//   validate: (date) =>
//     date.getTime() > Date.now() ? Effect.fail("Your birthdate can't be in the future") : Effect.succeed(date),
// });
...
const textPrompt = Prompt.text({
  ...
  type: 'hidden',
  validate: (value) =>
    value.length === 0 ? Effect.fail('must be non-empty\nyou entered ' + value) : Effect.succeed(value),
});
...
const prompt = Prompt.all([textPrompt]);
